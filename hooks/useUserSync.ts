...
export const useUserSync = () => {
  ...
  const setNeedsOnbording = useUserStore((state) => state.setNeedsOnbording);
    ...
    const syncUser = async () => {
      try {
        ...
        if (fetchError && fetchError.code !== "PGRST116") {
          ...
          setNeedsOnbording(true);
          return;
        }

        if (existingUser) {
          setCurrency(existingUser.currency ?? "INR");
          setNeedsOnbording(!existingUser.currency);
          return;
        }
        ...
        setNeedsOnbording(!newUser?.currency);
